...
def get_data():
    ...
    data=np.genfromtxt("1in_cubic.txt",delimiter="")
    shape = np.shape(data)
...
def training (inputs, outputs, bias_1, bias_2,width,number_hidden_neuron):
    global weight1
    global weight2
    
    
    weight1 = np.random.random((1,number_hidden_neuron))
    weight2 = np.random.random((number_hidden_neuron,1))
    
    lr = 0.1
    
    hActivation = np.zeros((number_hidden_neuron, 1), dtype=float)
    oActivation = np.zeros((1, 1), dtype=float)
     
    # outputs of neurons (after sigmoid function)
    iOutput = np.zeros((1, 1), dtype=float)      # +1 for bias
    hOutput = np.zeros((number_hidden_neuron, 1), dtype=float)  # +1 for bias
    oOutput = np.zeros((1), dtype=float)
     
    # deltas for hidden and output layer
    hDelta = np.zeros((number_hidden_neuron), dtype=float)
    ...
    for i in range(2000):
        for a in range(width):
            ...
            weight1 = weight1 - lr * np.dot(iOutput.transpose(),hDelta) 
            weight2 = weight2 - lr * np.dot(hOutput.transpose(),oDelta)
        i=0
...
if __name__ == '__main__':
    ...
    training(inputs,outputs,bias1,bias2,width,3)
    print(weight1)
    #sha = weight1.shape
    #print(sha)
    print(weight2)
