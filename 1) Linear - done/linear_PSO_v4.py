...
def init_variables(nb_neurons):
    ...
    weight_ih_train = np.random.uniform(low=-3, high=3, size=(1,nb_neurons)) # matrix
...
def function_chosen(inputs, weightih, weightho,activation_function):
    ...
    for a in range(width):
        ...
        in_outputs.append([iOutput,round(output,3)])
...
class swarm_particle :
    ...
    def init_pop(self, inputs, outputs, activation):         #FAUX FAUX FAUX FAUX FAUX
        ...
        for net_id in range(self.nb_pop):
            weight_ih_train = np.random.uniform(low=-3, high=3, size=(1,self.nb_neurons)) # matrix
...
if __name__ == '__main__':
        ...
        for a in range(10):
            ...
            for i in range(samples):
                ...
                particle=swarm_particle(nb_pop,a,nb_layer)
                epoch += particle.main(tolerated_error,epocmax,min_boundary, max_boundary)
            epoch = epoch / samples
            final.append(epoch)
        print(final)
        plt.plot(final,"g")
