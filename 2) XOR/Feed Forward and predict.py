...
for a in range (200):
    for i in range (width):
        ...
        ihidden_layer = np.dot(weight1, tmp_inputs) 
        #print(hidden_layer)
        ohidden_layer[0][0]= sigmoid(ihidden_layer[0][0])
        ohidden_layer[1][0]= sigmoid(ihidden_layer[1][0])
        #print(hidden_layer)
        
        
        ioutput_layer = np.dot(weight2, hidden_layer) 
        ooutput_layer = sigmoid(ioutput_layer)
        #print(output_layer)
        
        error_general = 0.5*(outputs_training[i] - output_layer)**2
