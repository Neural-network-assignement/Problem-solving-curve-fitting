...
FOR THIS COMPLEXE WE HAVE TO CREATE A NEW FCT 
init_pop adapted
...
def get_data():
    ...
    data=np.genfromtxt("2in_complex.txt",delimiter="")
...
def find_activation_function(inputs, outputs,weight_ih_train,weight_ho_train,nb_input):
    ...
    error_func = []
    samples = 70
    for i in range(2,5): # goind through every activation function
        error = 0
...
def see_function(inputs,outputs,activation): 
    ...
    for i in range(len(inputs)):
        xline.append(outputs[i][0])
        zline.append(inputs[i][0])
        yline.append(inputs[i][1])
...
##################################################################################################################    
    ...
    def update_velocity(self):
        ...
        w = 1.5#0.729 # inertia weight
        c1 = 1 #cognitive/local weight
        c2 = 2.5 # social/global weight
...
######################################################################################   
    def main(self, tolerated_error,epocmax,min_boundary, max_boundary,nb_input):
        ...
        print("result, format [input1,input2,outpu]:")
        print(output)
        """
